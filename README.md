...
The first thing is to determine which point is ocean and which point is land. In order to do that, i wrote program that randomly generated a continent map texture. Here are the steps:
...
Now generate a altitude map. The value at each point on the altitude map is generated by how far the pixel is to the top and bottom of the texture. So the center of the texture has values close to 1, and the top/bottom has the values close to 0.
...
To get the height values at each pixel, we first generate a M x N texture through compute shader, with each pixel's index remapped into longgitude and latitude. The coordinate is then transformed to positions on sphere as float3 (or vector 3). The position is then used to sample height values from a simplex noise function with fractal brownian motion.
...
After the generation of heightmap is complete, the height map texture is then read into memory as a nested array. Then the array is accessed in threads. The value of the vertex position woule be something like finalVert = originalVert * (1 + height_scalar * height) where height scalar is a very small number like 0.001, and height is the corresponding noise value.
...
The latter approach is better in practice. Even though i have not yet implemented this approach in this project, i have decided to use this approach in the project i'm currently working on.

One thing that i would like to mention is that, since i found a way to define biome boundaries, there is also a way to apply different height curves and blend different height curves between the biomes. For example, the shrubland biomes can on average have higher noise values, while the desert biome next to it can on average have lower values. 

The way i do it is to describe different biome's height curves as an animation curve. Where the t axis is the input noise values, and the y axis is the height values given the input noise value. After defining all the curves. I pre-sample all the curves in cpu and combine it into one compute buffer, then generate height values according to the compute buffer in gpu side.

There can also be blendings in between biomes, so that the height values between biomes are smooth and don't have some sort of wierd discontinuity. In order to blend height values across biomes, i will have to blend them in humidity-temperature map space, since that is how the boimes are defined. For example, the points with humidity value close to one boundary; we can set the height to be equal to some portion of the current biome plus some portion of the neighboring biome. I did not implement this in this project yet, and i will do this the my upcoming project. 
...
And here are two images of marching square details with wireframe on
...
Finally we have the "World". There are different seeds in the inspector, so you can change them to have a different world generation.

![SimpleWorld](Imgs/SimpleWorld.png)

---

#### There could be an ocean shader, better biome textures, some atmosphere compute shader, better lighting, and maybe a better terrain with marching cubes algorithm (with overhang mountains and ocean terrains), along with a lot better algorithm implementation + less spaghetti. And that is what i am working on next. I plan to build the next project into a actual program that can be run on windows and mac machines, and it would be like a very simple sandbox or survival-ish game, with a heavy focus on environment and procedural contents. The player would just swing arond the globe with a hook gun and explore the world.

#### My inspiration come from a youtuber named Sabastian Lague, go check out his channel!

---

Feel free to add me on discord if you find this interesting!
